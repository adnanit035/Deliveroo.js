...
class Agent extends Xy {
    ...
    constructor(grid, options) {
        ...
        Object.defineProperty(this, 'carrying', {
            get: () => Array.from(this.#carryingParcels).map(({id, reward}) => {
                return {id, reward};
            }), // Recursion on carriedBy->agent->carrying->carriedBy ...
            enumerable: false
        });
    ...
    async putDown(ids = []) {
        ...
        if (sc > 0) {
            ...
            console.log(Array.from(this.#grid.getAgents()).map(({
                                                                    name,
                                                                    id,
                                                                    score
                                                                }) => `${name}(${id}) ${score} pti`).join(', '));
        }
        if (dropped.length > 0)
